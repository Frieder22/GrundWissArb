...
Dabei sind $\hat{y}_i$ die Vorhersagen unseres Modells für die bekannten $x_i$ und $y_i$ die 'wahren' Daten, die wir aus unserem ursprünglichen Datensatz zu Verfügung haben. Da das Polynom $p(x)$ unsere Vorhersage zu einem Wert $x$ ist, ist $\hat{y}_i = p(x_i)$. Die Fehlerfunktion \ref{MSE} summiert daher die quadratischen Abstände zwischen ursprünglichen und vorhergesagten Werten auf und normiert sie mit der Anzahl der Datenpunkte $n$. Intuitiv erhalten wir daher nun 
...
Um Underfitting zu umgehen gibt es eigentlich nur eine Möglichkeit: Das Modell muss flexibler werden. Dazu muss meist die \emph{Komplexität} erhöht werden. Gemeint ist damit, dass sich die Anzahl der freien Parameter erhöht werden muss. In unserem Beispiel sind die freien Parameter die Faktoren $\{a_0, \dots, a_P\}$ der einzelnen Terme des Polynom. Daher steigt die Komplexität des Modells mit dem Grad des Polynoms. In der Art und Weise wie genau die Komplexität angepasst werden soll, liegt genau die in dem Essay zu diskutierende Kontroverse und wird im nächsten Abschnitt genauer dargestellt.\\
...
\section{Die unvollständige Idee eines globalen Minimums des Testfehlers in der Literatur}
Wie in dem vorherigen Abschnitt angeführten Vergleich erkennbar, hat die Modellkomplexität einen Einfluss auf den Testfehler. Es ist denkbar, dass es eine Region gibt, in welcher der Testfehler ein Minimum aufweist. Mann könnte erwarten, dass die optimale Modellkomplexität zwischen dem Bereich von Overfitting und dem Bereich von Underfitting liegt. Um diesen Punkt zu finden werden einfach verschiedene Modellkomplexitäten ausprobiert und der Testfehler dagegen aufgetragen. Mit der Idee eines Minimums im Testfehler würden wir eine Badewannenkurve erwarten. Eine solche Kurve ist auch vielen Lehrbüchern zu finden. Ein solcher Graph ist in Abbildung \ref{fig:Hastie} zu sehen. Im Anhang \ref{anhang} sind weitere Beispiele zu finden.
Dabei ist eines der Beispiele (Abbildung \ref{fig:Tuwien}) aus der Lehre und stammt aus einer Vorlesung des Sommersemesters 2023 zum Thema Machine Learning.
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\textwidth]{pics/Hastie.png}
	\caption{Darstellung des Testfehlers (rot) und des Trainingsfehlers (blau) in Abhängigkeit der Freiheitsgrade aus \cite{Hastie2009}. Die stark eingefärbten Linien beschreiben Mittelwerte der transparenten Linien.}
	\label{fig:Hastie}
\end{figure}

Die Abbildungen haben alle eine Gemeinsamkeit. In dem gezeigten Wertebereich ist nur ein lokales Minimum im Testfehler zu erkennen, welches auch in diesem Bereich dann ein globales Minimum darstellt. Allerdings stellt sich dieses Bild als unvollständig heraus.\\
In neueren Quellen (ab ca. 2019) wie beispielsweise durch Belkin et al. \cite{Belkin_2019} wird darauf hingewiesen, dass nach dem Anstieg des Testfehlers für höhere Modellkomplexität der Testfehler oft wieder abfällt. Dazu wird eine Grafik wie in Abbildung \ref{fig:doubleDescent} zur Erklärung verwendet.
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\textwidth]{pics/doubleDescent_scheme.png}
	\caption{Schematische Darstellung des doppelten Abstiegs (Double Descent) des Testfehlers. Die durchgezogene Linie gibt den Testfehler an und die gestrichelte Linie den Trainingsfehler. Die vertikale gepunktete Linie gibt die Trennung der 'alten' Idee mit Badewannenkurve (links) und des 'neuen' modernen Regimes (rechts) an.}
	\label{fig:doubleDescent}
\end{figure}
Diese Behauptung wird Anhand mehrerer Experimente auch validiert. Dabei werden auch unterschiedliche Arten von Modellen verwendet. Getestet werden unter anderem ein Random Fourier Features (RFF) Modell, ein neuronales Netzwerk und ein Random Forest Modell (Ergebnisse abgebildet in Anhang \ref{anhangb}). Dieses Phänomen wird dabei als \emph{Double Descent}, also doppelter Abstieg, beschrieben. Es gibt auch viele weitere Artikel, die Double Descent beschreiben \cite{DD1, DD2, DD3}. Darunter sind unter anderem Artikel von führenden AI Unternehmen wie beispielsweise OpenAI mit dabei \cite{openAI}. Double Descent wird aktiv genutzt und führt zu der aktuellen Strategie von AI Unternehmen viel komplexere Modelle zu verwenden. Man kann zu recht behaupten, dass das Phänomen in der aktuellen Forschung durchweg bekannt ist. Auch in neuerer Fachliteratur wird Double Descent beschrieben \cite{Hastie2009}. Andererseits ist der deutschen Wikipedia (\href{wikipedia.org}{wikipedia.org}) noch kein Artikel zu Double Descent zu finden (aufgerufen am 12.1.2026). In der englischen Variante ist dazu ein Artikel zu finden. Interessant ist hier aber auch, dass auf der Seite des alten Modells (genannt Bias-variance tradeoff: \href{en.wikipedia.org/wiki/Bias–variance_tradeoff}{en.wikipedia.org/wiki/Bias–variance\_tradeoff}, aufgerufen am 12.1.2026) im Text kein Hinweis auf das Phänomen Double Descent gegeben wird.\\
Es ist aber auch wichtig zu erwähnen, dass das alte Bild nicht komplett falsch ist. James et al. \cite{James2021} schreiben dazu beispielsweise, dass Double Descent dem Bias-Variance Tradeoff nicht widerspricht. Zum eines sei es eine Erweiterung des Bildes und zum anderen gibt es auch 'einfachere' Probleme, die das Phänomen des zweiten Abstiegs nicht aufweisen. \\
Eine Erklärung warum es diesen zweiten Abstieg überhaupt gibt, liefert unter anderen Belkin et al. \cite{Belkin_2019}. Als elementar wird die Idee von \emph{Ockhams Rasiermesser} angesehen. Sie sagt aus, dass die einfachsten Lösungen für ein Problem die besten und demnach auch die wahrscheinlichsten sind. Damit lässt sich zunächst auch der Anstieg der alten Denkweise und das Overfitting gut erklären. Wenn das Modell zu komplex ist, ist eine Generalisierung unwahrscheinlich. Für den darauffolgenden zweiten Abstieg ist dann aber noch eine weitere Beobachtung relevant. Er ist erst zu erwarten - und Experimente zeigen dies auch- , wenn der Trainingsfehler bei quasi null ist. Das heißt die Trainingsdaten werden hier von dem Modell bereits komplett perfekt beschrieben. Wenn das Modell also noch mehr Freiheitsgrade bekommt ist das System dann aber überbestimmt. Dem Modell sehen also nun mehrere Möglichkeiten zu Verfügung die Trainingsdaten perfekt zu bestimmen. Daher haben beispielsweise Regularisierungsterme ein großes Gewicht wodurch unnötige Parameter Richtung 0 'gedrückt' werden. \\
Wenn wir bei dem Beispiel dieses Essays (im genaueren der Polynomanpassung) bleiben, wissen wir, dass ein Polynom vom Grad $p$ durch $p+1$ Datenpunkte bereits eindeutig definiert ist. Wenn das Polynom aber nun einen wesentlich höheren Grad hat als es Datenpunkte gibt, gibt es mehrere Lösungen für eine Interpolation. Das Modell kann sich also nun aus diesem Pool von Lösungen die einfachste (das könnte beispielsweise die glatteste) Lösung 'aussuchen'. Daher kann das eigentlich komplexere Modell nun einfachere Lösungen finden, von denen wir oft ausgehen können, dass sie den ursprünglichen Sachverhalt besser erklären. Das zeigt sich dann im Absinken des Testfehlers.\\
Die Relevanz dieses Effekts ist offenkundig. Wenn wir beispielsweise die Abbildungen aus Anhang \ref{anhangb} betrachten, sehen wir, dass der Testfehler das lokale Minimum unterschreiten kann. Es können also Modelle gefunden werden, die besser generalisieren und neue unbekannte Datenpunkte besser klassifizieren. Das bedeutet, wenn das Training hier früher gestoppt worden wäre (gemeint ist hier keine Modelle mit höherer Komplexität zu betrachten nach einem Anstieg des Testfehlers), würden besser generalisierende Modelle außer acht gelassen.

\section{Zusammenfassung und Fazit}
Das alte Modell des Bias-Variance-Tradeoffs unvollständig und damit die Idee der Existenz eines einzigen lokalen Minimums des Testfehlers in vielen Fällen nicht richtig. Ältere Lehrbücher zeigen daher ein unvollständiges Bild. Auch in Lehrveranstaltungen und bei Wikipedia wird der Effekt des doppelten Abstiegs noch nicht hinreichend erwähnt. In neueren Lehrbüchern jedoch wird Double Descent als Thema behandelt. Der Effekt ist wichtig für die aktuelle Forschung und wird auch in wichtigen KI Unternehmen aktiv genutzt. Double Descent ist ein wichtiger Faktor für den Trend komplexere Modelle im Bereich KI und Machine Learning zu verwenden.\\
Das Forschungsfeld im Bereich Machine Learning entwickelt sich aktuell sehr schnell. Daher ist eine Diskrepanz zwischen Lehre und aktueller Forschung zu erwarten. Allerdings ist wurde der Effekt bereits 2019 in der Forschung diskutiert, ist aber beispielsweise 2023 noch nicht in einer passenden Vorlesung beschrieben. Auch das Nachhinken der deutschen Wikipedia (2025) ist erkennbar. Da aber Lehrbücher Double Descent als Thema bereits aufgreifen, ist zu erwarten, dass Lehre und online Enzyklopädie sich anpassen werden.\\
Wichtig zu erwähnen ist aber auch, dass einige Modelle und Datensätze keinen zweiten Abstieg des Testfehlers aufweisen. Die alte Denkweise behält dadurch auch seine Daseinsberechtigung. Es ergibt auch Sinn sie weiter in der Lehre zu verwenden, wobei aber ein Hinweis auf einen zweiten möglichen Abstieg des Testfehlers wichtig ist. 
...
\newpage
\appendix
\section{Beispiele mit nur einem lokalen Minimum} \label{anhang}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{pics/James.png}
	\caption{Darstellung des Testfehlers in Abhängigkeit der Modellkomplexität (auch Fleibilität) aus \cite{James2021}. Hier werden Testfehler von verschiedenen Validierungsmethoden verglichen (blau: naiver Testfehler, gestrichelt: LOOCV, orange: 10-fache Cross-Validation)}
	\label{fig:James}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{pics/Wang.png}
	\caption{Darstellung des Testfehlers in Abhängigkeit der Modellkomplexität aus \cite{Wang2025}.  Die rote Kennlinie stellt den Testfehler dar.}
	\label{fig:Wang}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{pics/TUWien.png}
	\caption{Darstellung des Testfehlers in Abhängigkeit der Modellkomplexität von der Vorlesung 'Machine Learning' des Sommersemesters 2023 an der TU Wien.}
	\label{fig:Tuwien}
\end{figure}

\section{Beispiele für Double Descent} \label{anhangb}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{pics/DD1.png}
	\caption{Double Descent erkennbar im Training eines Fully Connected Neural Network Modells. Dabei wurde einmal die zero-one Fehlerfunktion (links) und einmal MSE (rechts) als Fehlerfunktion verwendet \cite{Belkin_2019}.}
	\label{fig:DD1}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{pics/DD2.png}
	\caption{Double Descent erkennbar im Training eines Random Forest models \cite{Belkin_2019}.}
	\label{fig:DD2}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{pics/doubleDescent.png}
	\caption{Double Descent erkennbar im Training eines Random Fourier Features (RFF) Modells \cite{Belkin_2019}.}
	\label{fig:DD3}
\end{figure}
